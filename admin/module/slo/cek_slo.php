<script>
  function startCalc(){
    ...
    <?php
    $aksi="module/slo/cek_slo.php";
    $aksi2="module/cek_slo/cek_aksi.php";

    switch($_GET[aksi]){
      default:
      ?>
      <!----- ------------------------- MENAMPILKAN DATA MASTER barang ------------------------- ----->     
      <h3 class="box-title margin text-center">Data Sales Local Order</h3>
         ...
         <table id="example1" class="table table-bordered table-striped">
          <thead>
           <tr class="text-green">
            ...
            <th class="col-sm-2">No. SLO</th> 
            <th class="col-sm-1">Buat SLO</th>
          </tr>
        </thead>
...
// Tampilkan data dari Database
          $id_user=$_SESSION[id];
          $sql = "SELECT pesanan.no_surat,pesanan.id_pelanggan,pesanan.tgl_prospek,pelanggan.id_pelanggan, pelanggan.nama, pesanan.id_user FROM pesanan INNER JOIN pelanggan ON pesanan.id_pelanggan = pelanggan.id_pelanggan  ";
          ...
          while ($tampilkan = mysql_fetch_array($tampil)) { 
            ...
            <tr>
             ...
             <td><?php echo $tampilkan['no_slo']; ?></td>


            <td align="center">
               <a class="btn btn-xs btn-info" href="?module=cek_slo&aksi=lanjut&no_surat=<?php echo $tampilkan['no_surat'];?>" alt="Edit Data"><i class="glyphicon glyphicon-send"></i>Proses</a>
             </td>
<!-- 
             <td align="center">
              <a class="btn btn-xs btn-info" href="?module=cetak_penawaran" alt="Cetak Penawaran"><i class="glyphicon glyphicon-send"></i></a> 

               <button type="submit" name="submit" onclick="this.form.target='_blank';return true;" class="btn btn-xs btn-info"><i class="glyphicon glyphicon-send"></i>&nbsp; Proses</button>
             </td>
-->          
             <?php
           }
    ...
    <form class="form-horizontal" action="<?php echo $aksi2?>?module=cek_penawaran&aksi=edit" role="form" method="post" name="f1">      
      ...
      <div class="form-group">
        ...
        <div class="col-sm-5">
          ...
          <input type="text" class="form-control" readonly name="nama" value="<?php echo $tambah['nama']; ?>" >
      ...
      <div class="box box-solid box-warning">
        <div class="box-header">
          ...
          <span class="pull-right">
            <a class="btn btn-default" target="blank" href="#">
            ...
            while ($tampilkan = mysql_fetch_array($tampil)) { 
              ...
              <tr>
               ...
               <td><?php if  ( $blokir== 'N' ) {
                ...
                <td align="center">
                 <?php if ( $blokir== 'N' ) { ?>
                   <a class="btn btn-xs btn-success"  data-toggle="tooltip" title="ACC Barang??" href="<?php echo $aksi2 ?>?module=cek_penawaran&aksi=yes&id_barang=<?php echo $tampilkan['id_barang']; ?>" onclick="return confirm('Apakah anda yakin ingin ACC <?php echo $tampilkan['nama']; ?> ?')"><i class="glyphicon glyphicon-ok"></i></a>
                 <?php }
                 else { ?>
                   <a class="btn btn-xs btn-warning" data-toggle="tooltip" title="Batalkan Barang??" href="<?php echo $aksi2 ?>?module=cek_penawaran&aksi=no&id_barang=<?php echo $tampilkan['id_barang']; ?>" onclick="return confirm('Apakah anda mau membatalkan barang <?php echo $tampilkan['nama']; ?>?')"><i class="glyphicon glyphicon-remove"></i></a>
